...
/* Configure the peripherals common clocks */
  ...
  HAL_Delay(5000);
  //HAL_GPIO_WritePin(GPIOB, green_Pin, 0x00);
  //HAL_GPIO_WritePin(GPIOB, red_Pin, 0x01);

  /** Enable the WakeUp
  */
 // if (HAL_RTCEx_SetWakeUpTimer_IT(&hrtc, 0x7D0, RTC_WAKEUPCLOCK_RTCCLK_DIV16) != HAL_OK)		//1s sleep
 // {
	//printf("No error\n\r");
   // Error_Handler();
  //}
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) {
	if(GPIO_Pin == GPIO_PIN_4) {
		SystemClock_Config();
		HAL_ResumeTick();
		printf("Awake from External Interrupt\n\r");
