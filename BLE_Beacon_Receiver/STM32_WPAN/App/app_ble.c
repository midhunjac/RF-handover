              ...
              */
              ...
              if (event_type == ADV_NONCONN_IND)
              {
                /* ISOLATION OF BD ADDRESS AND LOCAL NAME */
   			   ...
   			   char address[3];
   			   for(int i=5; i>=0; i--) {
   				   //printf("%x", le_advertising_event->Advertising_Report[0].Address[i]);
   			   ...
   			   if(!strcmp(MAC_Address, "0080E121BEFA")) {
					...
					while(k < event_data_size)
					{
					  ...
					  //printf("Event type: %d     Event Data Size: %d     AD type: %d    RSSI:%d     \n\r", event_type, event_data_size, adtype, RSSI);
					  ...
					  switch (adtype)
					  {
						...
						case AD_TYPE_MANUFACTURER_SPECIFIC_DATA: /* Manufacturer Specific */
						  ...
						  { /* ST VERSION ID 01 */
							//APP_DBG_MSG("--- Temperature Beacon found --- \n\r");
						   printf("Device found. RSSI: %d\n\r", RSSI);
						   APP_DBG_MSG("-- SERVER DETECTED -- \n\r");
						   ...
						   //printf("Addr_type: %d     %x%x%x%x%x\n\r", SERVER_REMOTE_ADDR_TYPE, SERVER_REMOTE_BDADDR[0],SERVER_REMOTE_BDADDR[1],SERVER_REMOTE_BDADDR[2],SERVER_REMOTE_BDADDR[3],SERVER_REMOTE_BDADDR[4]);
						  }
						  break;
